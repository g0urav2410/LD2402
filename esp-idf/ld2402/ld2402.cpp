...
// Off (default): the AUTO behaviour below -- trust the module once it has
// proven it can report still, derive for modules that never do. On: always
// trust the module's raw byte, even before s_state_seen has proven it. A
// simple on/off rather than the three-way source switch tried earlier --
// that one added a whole second (range-gated, adaptively-thresholded)
// classifier alongside this one, and the adaptive part was never actually
// calibrated against this driver's real numbers, which is what broke
// "still" outright. This just changes which side of the *existing*,
// already-proven AUTO logic gets skipped.
static volatile bool s_force_module_only = false;
void ld2402_set_force_module_only(bool on) { s_force_module_only = on; }
bool ld2402_get_force_module_only(void) { return s_force_module_only; }
...
static void publishReading() {
    ...
    if (!s_force_module_only && !r.still && !(s_state_seen & (1u << LD2402_STATE_STILL))) {
